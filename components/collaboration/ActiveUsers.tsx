...
export function ActiveUsers() {
  const { currentUsers, isConnected } = useSocket()

  // Don't show if not connected or no other users
  if (!isConnected) {
    return null
  }

  // Total users includes current user + other users
    ...
    <div className="flex items-center gap-2">
      {currentUsers.size > 0 && (
        <div className="flex -space-x-2">
          {Array.from(currentUsers.entries()).slice(0, 5).map(([socketId, user], index) => (
          <div
            key={socketId}
      ...
      <span className="text-xs text-gray-500 dark:text-gray-400">
        {totalUsers === 1 ? 'Just you' : `${totalUsers} users online`}
      </span>
    </div>
  )
}